...
def get_embeddings_in_batches(docs, batch_size=100):
    ...
    for i in range(0, len(docs), batch_size):
        ...
        contents = [doc["question"] for doc in batch]
...
with open("/data/ephemeral/home/data/aug_data_x10.jsonl") as f:
    ...
    try:
        ...
        for idx, row in tqdm(test_query.iterrows(), total=len(test_query), desc="Processing queries"):
            ...
            if pd.notna(standalone_query) and isinstance(standalone_query, str) and len(standalone_query.strip()) > 0:
                ...
                # 1-1 one way 
                search_result = dense_retrieve(standalone_query, 50)
                docs = [
                ...
                # 중복되는 docid를 병합하여 question을 합치는 과정
                docid_dict = defaultdict(lambda: {"docid": None, "questions": [], "content": None})

                for doc in docs:
                    docid = doc["docid"]
                    docid_dict[docid]["docid"] = docid
                    docid_dict[docid]["content"] = doc["content"]
                    docid_dict[docid]["questions"].append(doc["question"])

                # 병합된 결과를 리스트 형태로 변환
                docs_merged = [
                    {"docid": value["docid"], "question": " ".join(value["questions"]), "content": value["content"]}
                    for value in docid_dict.values()
                ]
                ...
                pairs = [[standalone_query, doc['content']] for doc in docs_merged]
                reranker_model.eval()
                ...
                for _ in range(min(3, len(indices))):
                    ...
                    docid = docs_merged[idx]["docid"]
                    reference = {"score": float(-score), "question": docs_merged[idx]['question'], "content": docs_merged[idx]['content']}
                    topk.append(docid)
...
rescore_existing_results('/data/ephemeral/home/data/highscore.csv', "/data/ephemeral/home/sample_submission14_roberta_hybridmodified_synonyms_reranking_q10.csv")

# MAP Score: 0.7517676767676765 MAP Score: 0.7880880230880226
#rescore_existing_results("/data/ephemeral/home/sample_submission6_roberta_sparse_fullprompt.csv", "/data/ephemeral/home/sample_submission14_roberta_hybridmodified_synonyms_reranking_q10.csv")

       ...
       # 각 row의 standalone_query 컬럼에 대해 검색 수행
        for idx, row in test_query.iterrows():
            ...
            if pd.notna(standalone_query) and isinstance(standalone_query, str) and len(standalone_query.strip()) > 0:
                ...
                search_result = hybrid_retrieve(standalone_query, 20)
                
                cross_encoder_model = CrossEncoder("jhgan/ko-sroberta-multitask")
                
                # scores = CrossEncoder_model.predict([
                ...
                score_passages = []
                for rst in search_result['hits']['hits']:
                    docid = rst.get("_source").get("docid")
                    passage = rst.get("_source").get("content")
                    score_passages.append((docid, passage))
                
                # Cross Encoder로 스코어링
                if score_passages:
                    scores = cross_encoder_model.predict([(standalone_query, passage) for _, passage in score_passages])
                    scored_results = sorted(zip(scores, score_passages), key=lambda x: x[0], reverse=True)
                    
                    # 상위 3개 결과 추출
                    top_3_results = scored_results[:3]
                    
                    for score, (docid, content) in top_3_results:
                        print("*"*10)
            ...
            #         if 'highlight' in rst:
            #             response["highlight"].append(rst['highlight'])
            #         if 'human' in rst:
            #             response["human"].append(rst['human'])
            #         if 'track_scores' in rst:
            #             response["track_scores"].append(rst['track_scores'])
            #         if '_explanation' in rst:
            #             response["explanation"].append(rst['_explanation'])
            #     updated_results.append(response)
            # else:
            #     response = {"eval_id": row.get("eval_id"), "standalone_query": standalone_query, "topk": [], "references": []}
            #     updated_results.append(response)
...
rescore_existing_results("/data/ephemeral/home/sample_submission6_roberta_llmchoice_fullprompt.csv", "/data/ephemeral/home/sample_submission13_roberta_hybrid_crossencoder.csv")
